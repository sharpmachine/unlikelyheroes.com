...
*************************************************************************/
$meta_boxes = array();
$meta_boxes = apply_filters ( 'ign_cmb_meta_boxes' , $meta_boxes );
...
define( 'CMB_META_BOX_URL', plugins_url('/ign_metabox/', dirname(__FILE__)));
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'text_date':
					...
					break;
				case 'text_money':
					echo '<input class="cmb_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="'.number_format( ($meta ? $meta : $field['std']), 2, '.', ','). '" /> ';
					break;
				case 'textarea':
				...
				case 'textarea_medium':
					echo '<textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="7" style="width:97%">', $meta ? $meta : $field['std'], '</textarea>',
						' ';
					break;					
				case 'select':
...
*/
				...
				case 'product_levels':
					...
					if ($meta_no_levels > 0 || $meta_no_levels != "") {
						echo '<div levels="'.$meta_no_levels.'"><input id="levels" name="level-count" type="hidden" value="'.$meta_no_levels.'"/>';
						...
						for ($i=2 ; $i <= $meta_no_levels ; $i++) {
							...
							echo '<div'.(($i == 0) ? '' : ' level="'.($i).'"').' class="projectmeta-levelbox">'.
									...
									'<div class="ign_projectmeta_reward_limit"><label style="font-weight: bold">'.$GLOBALS["tr_Limit_For"].' </label><input class="cmb_text_small" type="text" name="levels['.$i.'][limit]" id="ign_level_'.$i.'_limit" value="'.$meta_limit.'" /></div>'.
									'<div class="ign_projectmeta_reward_limit"><label style="font-weight: bold">'.$GLOBALS["tr_Level_Order"].' </label><input class="cmb_text_small" type="text" name="levels['.$i.'][order]" id="ign_level_'.$i.'_order" value="'.$meta_order.'" /></div></div>' .
									'<div class="ign_projectmeta_reward_desc"><label style="font-weight: bold">'.$GLOBALS["tr_Short_Description_For"].' </label><textarea name="levels['.$i.'][short_description]" id="ign_level'.$i.'short_desc" cols="60" rows="4" style="width:97%">'.$meta_short_desc.'</textarea></div>'.
									'<div class="ign_projectmeta_reward_desc"><label style="font-weight: bold">'.$GLOBALS["tr_Long_Description_For"].' </label><textarea name="levels['.$i.'][description]" id="ign_level'.$i.'desc" cols="60" rows="4" style="width:97%">'.$meta_desc.'</textarea></div>'.
								'<div></div>';
								echo '</div>';
						}
						
					} else {
						echo '<div levels="1"><input id="levels" name="level-count" type="hidden" value="1"/>';
	...
	function save($post_id, $post)  {
		...
		//===================================================================
			...
			if ($_POST['ign_product_price'] == "") {
				...
				$product_price = $_POST['ign_product_price'];
			}
			...
			if ($post->post_type == 'ignition_product') {
				if ( stristr($_SERVER['HTTP_REFERER'], "action=edit") !== false) {
					...
					$product_price = round(str_replace(",", "", $product_price), 2);		//replacing commas with empty
					...
					$sql_update = $wpdb->prepare("UPDATE ".$wpdb->prefix."ign_products SET product_name = %s,
						...
						product_price = %s,
						product_url = %s,
						goal = %s WHERE id = %d", $_POST['ign_product_name'], $_POST['ign_product_title'], $_POST['ign_product_limit'], $_POST['ign_product_details'], $product_price, $_POST['id_project_URL'].'/', $product_goal, $project_id);
					...
					$sql_product = $wpdb->prepare("INSERT INTO ".$wpdb->prefix ."ign_products (
									...
									created_at) VALUES (
										...
										%s,
										'".date('Y-m-d H:i:s')."'
									)", $_POST['ign_product_name'],
									$_POST['id_project_URL'].'/' ,
									$_POST['ign_product_title'] , 
									$_POST['ign_product_limit'], 
									$_POST['ign_product_details'],
									$product_price,
									$product_goal);
		...
		jQuery(document).ready(function(e) {
			jQuery('span[addlevel]').click(function () {
				...
				jQuery('div[levels]').append('<div level="'+element_number+'" class="projectmeta-levelbox">' +
										'<h2><?php echo $GLOBALS['tr_Level']; ?>'+(element_number)+' </h2><div class="ign_projectmeta_reward_title"><label><?php echo $GLOBALS["tr_Title_For"]; ?> </label><input class="cmb_text" type="text" name="levels['+element_number+'][title]" id="ign_level'+element_number+'title" value="" /></div>'+
										'<div class="ign_projectmeta_reward_left"><div class="ign_projectmeta_reward_price"><label class="cmb_metabox_description"><?php echo $GLOBALS["tr_Price_For"]; ?> </label><input class="cmb_text_money" type="text" name="levels['+element_number+'][price]" id="ign_level'+element_number+'" value="" /></div>' +
										'<div class="ign_projectmeta_reward_limit"><label class="cmb_metabox_description"><?php echo $GLOBALS["tr_Limit_For"]; ?> </label><input class="cmb_text_small" type="text" name="levels['+element_number+'][limit]" id="ign_level'+element_number+'limit" value="" /></div>' +
										'<div class="ign_projectmeta_reward_limit"><label class="cmb_metabox_description"><?php echo $GLOBALS["tr_Level_Order"]; ?> </label><input class="cmb_text_small" type="text" name="levels['+element_number+'][order]" id="ign_level'+element_number+'order" value="0" /></div></div>' +
										'<div class="ign_projectmeta_reward_desc"><label style="font-weight: bold"><?php echo $GLOBALS["tr_Level"]; ?> ' + element_number + ' <?php echo $GLOBALS["tr_Short_Description_For"]; ?> </label><textarea name="levels['+element_number+'][short_description]" id="ign_level'+element_number+'short_desc" cols="60" rows="4" style="width:97%"></textarea></div>' +
									'<div class="ign_projectmeta_reward_desc"><label style="font-weight: bold"><?php echo $GLOBALS["tr_Level"]; ?> ' + element_number + ' <?php echo $GLOBALS["tr_Long_Description_For"]; ?> </label><textarea name="levels['+element_number+'][description]" id="ign_level'+element_number+'desc" cols="60" rows="4" style="width:97%"></textarea></div>' +
