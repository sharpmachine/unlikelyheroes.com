			...
			<div class="product-wrapper">
				...
				<div class="pledge">
					<?php  if (!$custom || ($custom && isset($attrs['project_title']))) { ?>
						<h2 class="id-product-title"><a href="<?php echo getProjectURLfromType($project_id); ?>"><?php echo stripslashes($the_deck->project->product_name);?></a></h2>
					<?php } ?>
				...
				<?php if (!$custom || ($custom && isset($attrs['project_pledged']))) { ?>
					<div class="id-progress-raised"> <?php echo $the_deck->cCode; ?><?php echo number_format(apply_filters('id_funds_raised', $the_deck->p_current_sale, $the_deck->post_id)); ?> </div>
				<?php } ?>
				<?php if (!$custom || ($custom && isset($attrs['project_goal']))) { ?>
					<div class="id-product-funding"><?php echo $tr_Pledged_Of; ?> <?php echo $the_deck->cCode;?><?php echo number_format($the_deck->project->goal);?> <?php echo $tr_Goal; ?></div>
				<?php } ?>
