<?php 
	...
	apply_filters('manage-order_columns', $columns);
		...
		foreach ($products as $product) {
			$product_array[$product->id] = $product->product_name;
		}
	...
	<div class="tablenav">
	    <div class='tablenav-pages'>
	    	...
	    	<div class="alignleft actions">
	    		<select name="project">
	    			<option value="0">Show all projects</option>
	    			<?php
	    			foreach ($prod_name_id_array as $prod_combo) {
	    				echo '<option value="'.$prod_combo['id'].'" name="prod'.$prod_combo['id'].'" '.(isset($_GET['project']) && $prod_combo['id'] == $_GET['project'] ? 'selected="selected"' : "").'>'.stripslashes($prod_combo['product_name']).'</option>';
	    			}
	    			?>
	    		</select>
