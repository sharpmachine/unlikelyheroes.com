...
function is_id_network_activated() {
	...
	if (isset($active_plugins['ignitiondeck/ignitiondeck.php'])) {
		if (is_multisite()) {
			return true;
		}
	}
...
function ign_pre_install ($blog_id = null) {
	...
	prod_page_adn TINYINT( 1 ) NOT NULL,
...
function adaptivePreapproval() {
	if (isset($_POST['btnSubmitPreapproval'])) {
		...
		$preapproval_key = "";
		$product_name = getProductDetails($payment_variables['product_id']);
		...
		$preauth->endingDate = date("Y-m-d\Z", strtotime("+ 365 day"));
...
function embedWidget() {
	...
	$sql = "SELECT * FROM ".$wpdb->prefix."ign_products WHERE id = '".$_GET['product_no']."'";
	$product = $wpdb->get_row( $sql );
	//print_r($product)."<br />";
	if (!empty($product)) {
		$project_id = $product->id;
		$project = new ID_Project($project_id);
		$post_query = "SELECT * FROM ".$wpdb->prefix."postmeta WHERE meta_key = 'ign_project_id' AND meta_value = '".absint($product->id)."'";
		$post_res = $wpdb->get_row( $post_query );
		$post_id = $post_res->post_id;

		$item_fund_goal = get_post_meta($post_id, 'ign_fund_goal', true);
		$item_fund_end = get_post_meta($post_id, 'ign_fund_end', true);
		...
		$rating_per = 0;
		if ($p_current_sale > 0) {
			$rating_per = number_format($p_current_sale/$product->goal*100, 2,'.', '');
		}

		$days_left = str_replace("/", "-", $item_fund_end);
		...
		//GETTING product default settings
		$default_prod_settings = $project->get_project_defaults();
