...
function subscribeToMailchimp($email, $userData, $productId){
    $mailchimpSettings = getProductMailchimpSettings($productId);
	if (empty($mailchimpSettings)) {
		$mailchimpSettings = getMailchimpSettings();
		if (isset($mailchimpSettings)) {
			$api_key = $mailchimpSettings->api_key;
			$list_id = $mailchimpSettings->list_id;
		}
	} else {
		$api_key = $mailchimpSettings->mailchimp_api_key;
		$list_id = $mailchimpSettings->mailchimp_list_id;
	}
    $merges = array(
        'FNAME'=>$userData['first_name'],
        'LNAME'=>$userData['last_name']
    );
	if (isset($api_key) && isset($list_id)) {
		require_once('inc/MCAPI.class.php');
		$api = new MCAPI($api_key);
		
		$added_to_mailchimp = $api->listSubscribe($list_id, $email, $merges);
		/*echo '<script type="text/javascript">alert("'.$added_to_mailchimp.'")</script>';*/
		
	    $data = array(
	        'email_address'=> $email,
			'FNAME'=> $userData['first_name'],
			'LNAME'=> $userData['last_name'],
	        'apikey'=> $api_key/*$mailchimpSettings->api_key*/,
			'merge_vars' => $merges,
			'id' => $list_id/*$mailchimpSettings->list_id*/,
	        'double_optin' => true,
	    ...
	    $data = json_decode($result);
	   	/** if($data->error){
	        ...
	        echo "<b><font color='#FF0000'>Success, look for the confirmation message\n</font></b>";
	    } */
	}
}
...
function getimagepostbyProductID($project_id) {
	global $wpdb;
	$project = new ID_Project($project_id);
	$post_id = $project->get_project_postid();	
	...
	$res = $wpdb->get_row($sql);
	if (isset($res->ID)) {
		$src = wp_get_attachment_image_src($res->ID, 'id_embed_image');
		$image = $src[0];
	} 
	else {
		$image = $url;
...
function getProductURL($prod_id, $paypal_page="") {
	...
	if (get_option('permalink_structure') == "") {
		...
		if ($paypal_page == "")
			$product_url = site_url()."/?ignition_product=".$post->post_name;
		else
			$product_url = site_url()."/?ignition_product=".$post->post_name."&paypa_passed=yes";
	} else {
		$post = getPostDetailbyProductID($prod_id);
		if ($paypal_page == "")
			$product_url = site_url()."/projects/".$post->post_name;
		else
			$product_url = site_url()."/projects/".$post->post_name."/?paypa_passed=yes";
	}
			...
			if (get_option('permalink_structure') == "") {
				if (get_post_meta($post->ID, 'ign_option_project_url', true) == "current_page") {		// If Project URL is the normal Project Page
					if ($page == "")
						...
						$product_url = site_url()."/?ignition_product=".$post->post_name."&purchaseform=1&prodid=".$project_id;
					else if ($page == "preapprovalkey")
						$product_url = site_url()."/?ignition_product=".$post->post_name."&generatepreapproval=1";
					else
						$product_url = site_url()."/?ignition_product=".$post->post_name."&paypa_passed=yes";
				...
				if (get_post_meta($post->ID, 'ign_option_project_url', true) == "current_page") {		// If Project URL is the normal Project Page
					if ($page == "")
						...
						$product_url = site_url()."/projects/".$post->post_name."/?purchaseform=1&prodid=".$project_id;
					else if ($page == "preapprovalkey")
						$product_url = site_url()."/projects/".$post->post_name."/?generatepreapproval=1";
					else
						$product_url = site_url()."/projects/".$post->post_name."/?paypa_passed=yes";
...
function getProductSettings($product_id) {
	...
	$sql_settings = "SELECT * FROM ".$wpdb->prefix."ign_product_settings WHERE product_id = '".$product_id."'";
	$settings = $wpdb->get_row( $sql_settings );
	
	return $settings;
}

function getProductInProductPage() {
	global $wpdb;
	
	if (get_option('permalink_structure') == "") {
		$post_name = $_GET['ignition_product'];
		$args = array(
			'name' => $post_name,
		...
		foreach ($my_posts as $post) {
			$product_name = mysql_real_escape_string(get_post_meta($post->ID, 'ign_product_name', true));
		}
		$sql = "SELECT * FROM ".$wpdb->prefix . "ign_products WHERE LOWER(product_name) = '".$product_name."' limit 0,1";
	} else {
		if (substr($_SERVER['REQUEST_URI'], -1, 1) == "/")
			$post_name = substr($_SERVER['REQUEST_URI'], 0, -1);

		$post_name = end(explode("/", $post_name));
...
function getProductIDFromProductName($name) {
	global $wpdb;
	$sql = $wpdb->prepare('SELECT id FROM '.$wpdb->prefix.'ign_products WHERE product_name=%s', $name);
	$res = $wpdb->get_row($sql);
	if ($res) {
		return $res->id;
	}
		...
		if (isset($email)) {
			if (isset($product_settings)) {
				...
				if ($active_mailtype == 'mailchimp') {
					subscribeToMailchimp($email, array('first_name' => $first_name, 'last_name' => $last_name ), $pay_info_id);
				}
				else if ($active_mailtype == 'aweber') {
					subscribeToAweber($email, array('first_name' => $first_name, 'last_name' => $last_name ), $order->product_id);
				}
			}
			else {
				if (isset($mailchimp_settings) && $mailchimp_settings->is_active) {
					subscribeToMailchimp($email, array('first_name' => $first_name, 'last_name' => $last_name ), $pay_info_id);
				}
				else if (isset($aweber_settings) && $aweber_settings->is_active) {
					subscribeToAweber($email, array('first_name' => $first_name, 'last_name' => $last_name ), $order->product_id);
				}
			}
		}
