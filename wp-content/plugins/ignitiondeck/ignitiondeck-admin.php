...
/* Content */
add_action('manage-order_content', 'order_detail_content');

function order_detail_content($column_name, $id) {
	global $post;
	require 'languages/text_variables.php';
	switch ($column_name) {
		//Project Name
	case 'project':
		...
		break;
	case 'level':
		echo $item->product_level;
		break;
	case 'pledged':
		echo $item->prod_price;
		break;
	case 'date':
		...
		if ($_POST['btnIgnSettings'] == $tr_Add) {		
			$sql_insert = "	INSERT INTO ".$wpdb->prefix."ign_settings
							(
								...
								'".(isset($_POST['prod_page_adn']) ? $_POST['prod_page_adn'] : 0)."',
			...
			$sql_update = "	UPDATE ".$wpdb->prefix."ign_settings SET
							...
							prod_page_adn = '".(isset($_POST['prod_page_adn']) ? $_POST['prod_page_adn'] : 0)."',
...
function view_order() {
	...
	$sql_prod = "SELECT * FROM ".$wpdb->prefix."ign_products WHERE id = '".$order_data->product_id."'";
	$product_data = $wpdb->get_row( $sql_prod );
	
	$post_id = getPostbyProductID($order_data->product_id);
...
function add_order() {
	...
	if ( isset($_POST['btnAddOrder']) ) {
		...
		/**
		This is all moved to the hook in functions file
		*/
		/*
		if ($product_settings != -1 && $product_settings->active_mailtype == "mailchimp")
			subscribeToMailchimp($_POST['email'], array('first_name' => $_POST['first_name'], 'last_name' => $_POST['last_name'] ), $_POST['product_id']);
		else if ($mailchip_settings->is_active)
			subscribeToMailchimp($_POST['email'], array('first_name' => $_POST['first_name'], 'last_name' => $_POST['last_name'] ), $_POST['product_id']);
			
		if ($product_settings != -1 && $product_settings->active_mailtype == "aweber")
			subscribeToAweber($_POST['email'], array('first_name' => $_POST['first_name'], 'last_name' => $_POST['last_name'] ), $_POST['product_id']);
...
function main_settings() {
	...
	if ($_POST['btnIgnSettings'] == $tr_Add) {	
		...
		$sql_update = "	UPDATE ".$wpdb->prefix."ign_settings SET
						...
						prod_page_adn = '".$_POST['prod_page_adn']."',
...
function id_settings_notice() {
	echo '<div class="updated">
	       <p>IgnitionDeck is active. Please <a href="wp-admin/admin.php?page=ignitiondeck">save settings</a> before creating your first project. | <a href="#" id="id_settings_notice" class="hide-notice">Hide Notice</a></p>
	    </div>';
}
...
function id_products_notice() {
	echo '<div class="updated">
	       <p>IgnitionDeck is active. Now it&rsquo;s time to <a href="post-new.php?post_type=ignition_product">create your first project</a>. | <a href="#" id="id_products_notice" class="hide-notice">Hide Notice</a></p>
	    </div>';
}
