		...
		<div id="user-list">
			<div class="tablenav top">
				...
				<?php foreach ($levels as $level) {?> 
					<div class="user-stat"><?php echo '<a class="'.(isset($_GET['level']) && $_GET['level'] == $level->id ? 'active' : '').'" href="admin.php?'.(isset($query_string) ? $query_string : '').'&amp;level='.urlencode($level->id).'"><div class="user-title" style="display: inline-block;">'.$level->level_name.'</div><div class="user-number" style="display: inline-block;">'.$level->count.'</div></a>'; ?></div>
				<?php } ?>
				</div>
				<div class="tablenav-pages"><span class="displaying-num"><?php echo $pages; ?> <?php _e('items', 'memberdeck'); ?></span>
					<?php if (isset($page) && $page > 1) { ?>
					<span class="pagination-links"><a class="first-page disabled" title="<?php _e('Go to the first page', 'memberdeck'); ?>" href="admin.php?<?php echo (isset($query_first) ? $query_first : ''); ?>">«</a>
					<a class="prev-page disabled" title="<?php _e('Go to the previous page', 'memberdeck'); ?>" href="admin.php?<?php echo (isset($query_next) ? $query_prev : ''); ?>">‹</a>
					<?php } ?>
